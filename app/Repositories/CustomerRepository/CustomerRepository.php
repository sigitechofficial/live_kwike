...
use App\Models\SearchingKeyword;
...
class CustomerRepository implements CustomerRepositoryInterface
{
    ...
    public function getNearestStore($request)
    {
        ...
        $store->service_fee='10.00';
        $store->delivery_fee='5.00';
    ...
    public function addToCart($request)
    {
        ...
        if (count($haveProducts) > 0) {
            ...
            if (count($cart_items) > 0){
                ...
                $cart->delete();
                response()->json(['ResponseCode' => '0', 'ResponseMessage' => 'Something went wrong.!', 'Response' => $data,'errors'=>['Store product is out stock']], 200)->send();
            }
            return $data;
        }
        return 0;
...
//            dd(env('STRIPE_SECRET_KEY'),env('IMAGE_URL'));
            $cart = Cart::where('user_id', auth()->id())->first();
            if (!$cart) {
                errorResponse('0', 'Something went wrong.!', ['You have no products in your cart'], 200);
            }
            ...
            if ($cart) {
                ...
                $orderNo = 'kwikemart-' . rand(10000, 99999);
                $order = Order::create([
                    'order_no' => $orderNo,
                    'user_id' => auth()->id(),
                    ...
                    'total' => $cart->total,
                    'schedule_date' =>$this->TimeZoneToUTC($request->schedule_date,auth()->user()->time_zone),
                ...
                foreach ($cart->cartItems as $item) {
                    ...
                    if (!$productStore->stock >= $item->quantity) {
                        DB::rollBack();
                        errorResponse('0', 'Something went wrong', ['Cart Product not available'], 200);
                    }
            ...
            DB::rollBack();
            errorResponse('0', 'Server error', [$e->getMessage()],200);
        }

    }
    public function ordersList($request)
    {
        $orders=Order::with('orderItems')->where('user_id',auth()->id())->where('status',$request->get('status'))->get();
    ...
    {
        $order=Order::with('userAddress','orderItems.productStore.products')->findOrFail($request->get('id'));
        $order=UserOrderDetailResource::make($order);
        $data=['image_url'=>env('IMAGE_URL'),'order'=>$order];
        return $data;
    }
    public function SearchProducts($request)
    {
        $products = ProductStore::with('products')->whereHas('products',function ($query) use($request){
            $query->where('title', 'like', '%'.$request->keyword.'%');
        })->where(['store_id' => $request->get('store_id')])->get();
        $products=SearchProductResource::collection($products);
        return ['images_url' => env('IMAGE_URL'),
            'products'=>$products
            ];
    }
    public function searchKeywordList()
    {
        $keywords=SearchingKeyword::all();
        return ['kewords'=>$keywords];
    }
}
